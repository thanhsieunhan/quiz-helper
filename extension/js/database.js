...
async function loadDatabase(page = 1) {
  try {
    ...
    async function filterQuestions() {
      ...
      const questionIdTerm = questionIdInput.value;

      try {
        // Gọi API tìm kiếm với các tham số
        const response = await fetch(
          `${API_URL}/questions/?search=${encodeURIComponent(
            searchTerm,
          )}&question_id=${encodeURIComponent(
            questionIdTerm,
          )}&page=1&page_size=${currentPageSize}`,
        );
        ...
        if (data.questions.length === 0) {
          database.innerHTML = '<p>Không tìm thấy câu hỏi nào phù hợp</p>';
          return;
        }
        ...
        data.questions.forEach((question, index) => {
          ...
          questionDiv.className = 'question-item';
          questionDiv.innerHTML = `
            <p><strong>${index + 1}. Câu hỏi:</strong> ${question.content}</p>
            <p><strong>ID:</strong> ${question.question_id}</p>
            <p><strong>Đáp án:</strong></p>
            <ul>
              ${question.answers
        ...
        // Thêm lại event listener cho pageSizeSelect
        const pageSizeSelect = document.getElementById('pageSizeSelect');
        pageSizeSelect.addEventListener('change', function (e) {
          currentPageSize = parseInt(e.target.value);
          currentPage = 1;
        ...
        document.getElementById(
          'database',
        ).innerHTML = `<p style="color: red;">Lỗi khi tìm kiếm: ${error.message}</p>`;
      }
    }
    ...
    questionIdInput.addEventListener('input', debouncedFilter);
  } catch (error) {
    document.getElementById(
      'database',
